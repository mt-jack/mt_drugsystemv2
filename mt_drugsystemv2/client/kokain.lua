			...
			for k in pairs(locations) do
				if GetDistanceBetweenCoords(locations[k].x, locations[k].y, locations[k].z, GetEntityCoords(GetPlayerPed(-1))) < 50 and Config.EnableCoke then	
					...
					if IsControlJustReleased(0, Keys['E']) then
						sleep = 5
						Pickingup()
						TriggerEvent('mt_coke:Refresh', k)
			...
			if GetDistanceBetweenCoords(Config.Processing.x, Config.Processing.y, Config.Processing.z, GetEntityCoords(GetPlayerPed(-1))) < 150 and Config.EnableCoke then			
					...
					if GetDistanceBetweenCoords(Config.Processing.x, Config.Processing.y, Config.Processing.z, GetEntityCoords(GetPlayerPed(-1)), true) < 2 then			
						...
						if IsControlJustReleased(0, Keys['E']) and not blip then
								...
								Citizen.Wait(5000)
								blip = true
								TriggerServerEvent('mt_coke:ProcessItems')
...
function Pickingup()
	...
	if IsPedInAnyVehicle(GetPlayerPed(-1)) then
		...
		Citizen.Wait(2700)
		TriggerServerEvent('mt_coke:Pickup')
		ClearPedTasks(ped)
		FreezeEntityPosition(ped,false)
	end
end

RegisterNetEvent('mt_coke:marker')
AddEventHandler('mt_coke:marker', function()
	local set = false
	Citizen.Wait(10)
...
RegisterNetEvent('mt_coke:Refresh')
AddEventHandler('mt_coke:Refresh', function(id)
	local set = false
	Citizen.Wait(10)
	local rnX = Config.PickupBlip.x + math.random(-35, 35)
	local rnY = Config.PickupBlip.y + math.random(-35, 35)
