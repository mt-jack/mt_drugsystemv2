			...
			if GetDistanceBetweenCoords(locationsOpium[k].x, locationsOpium[k].y, locationsOpium[k].z, GetEntityCoords(GetPlayerPed(-1))) < 50 and Config.Enableopium then
				...
				if GetDistanceBetweenCoords(locationsOpium[k].x, locationsOpium[k].y, locationsOpium[k].z, GetEntityCoords(GetPlayerPed(-1))) < 3 then
					...
					if IsControlJustReleased(0, Keys['E']) then
						sleep = 5
						PickingupOpium()
						TriggerEvent('mt_opium:Refresh', k)
					end
				end
			end
		end
		...
		if GetDistanceBetweenCoords(Config.ProcessingOpium.x, Config.ProcessingOpium.y, Config.ProcessingOpium.z, GetEntityCoords(GetPlayerPed(-1))) < 150 and Config.Enableopium then
			...
			if GetDistanceBetweenCoords(Config.ProcessingOpium.x, Config.ProcessingOpium.y, Config.ProcessingOpium.z, GetEntityCoords(GetPlayerPed(-1)), true) < 2 then
				...
				if IsControlJustReleased(0, Keys['E']) and not blip then
					...
					Citizen.Wait(5000)
					blip = true
					TriggerServerEvent('mt_opium:ProcessItems')
...
function PickingupOpium()
	...
	if IsPedInAnyVehicle(GetPlayerPed(-1)) then
		...
		Citizen.Wait(2700)
		TriggerServerEvent('mt_opium:Pickup')
		ClearPedTasks(ped)
		FreezeEntityPosition(ped,false)
	end
end

RegisterNetEvent('mt_opium:marker')
AddEventHandler('mt_opium:marker', function()
	local set = false
	Citizen.Wait(10)
...
RegisterNetEvent('mt_opium:Refresh')
AddEventHandler('mt_opium:Refresh', function(id)
	local set = false
	Citizen.Wait(10)
	local rnX = Config.PickupBlipOpium.x + math.random(-35, 35)
	local rnY = Config.PickupBlipOpium.y + math.random(-35, 35)
