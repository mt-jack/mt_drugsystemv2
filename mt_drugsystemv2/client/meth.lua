			...
			for k in pairs(locationsMeth) do
				if GetDistanceBetweenCoords(locationsMeth[k].x, locationsMeth[k].y, locationsMeth[k].z, GetEntityCoords(GetPlayerPed(-1))) < 50 and Config.Enablemeth then	
					...
					if IsControlJustReleased(0, Keys['E']) then
						sleep = 5
						PickingupMeth()
						TriggerEvent('mt_meth:Refresh', k)
			...
			if GetDistanceBetweenCoords(Config.ProcessingMeth.x, Config.ProcessingMeth.y, Config.ProcessingMeth.z, GetEntityCoords(GetPlayerPed(-1))) < 150 and Config.Enablemeth then			
					...
					if GetDistanceBetweenCoords(Config.ProcessingMeth.x, Config.ProcessingMeth.y, Config.ProcessingMeth.z, GetEntityCoords(GetPlayerPed(-1)), true) < 2 then			
						...
						if IsControlJustReleased(0, Keys['E']) and not blip then
								...
								Citizen.Wait(5000)
								blip = true
								TriggerServerEvent('mt_meth:ProcessItems')
...
function PickingupMeth()
	...
	if IsPedInAnyVehicle(GetPlayerPed(-1)) then
		...
		Citizen.Wait(2700)
		TriggerServerEvent('mt_meth:Pickup')
		ClearPedTasks(ped)
		FreezeEntityPosition(ped,false)
	end
end

RegisterNetEvent('mt_meth:marker')
AddEventHandler('mt_meth:marker', function()
	local set = false
	Citizen.Wait(10)
...
RegisterNetEvent('mt_meth:Refresh')
AddEventHandler('mt_meth:Refresh', function(id)
	local set = false
	Citizen.Wait(10)
	local rnX = Config.PickupBlipMeth.x + math.random(-35, 35)
	local rnY = Config.PickupBlipMeth.y + math.random(-35, 35)
